...
class projectTimeTable extends Component {
  ...
  render() {
    return (
      <div>
        <h4>
          Time entries for {localStorage.getItem("client_name")} on{" "}
          {localStorage.getItem("project_name")} project
        </h4>
        <button
        ...
        <table className="table">
          ...
          <tbody>
            {this.state.data.map((time) => (
              <tr>
                <th scope="row">
                  {moment.utc(time.date_of_service).format("ll")}
                </th>
                <td>{time.hours}</td>
